...
from .nodalDG import GradVandermonde2D, Nodes2D, Vandermonde2D, xytors
...
x, y = 0.5 * (
    -np.dot(vertices[0].reshape(-1, 1), [r + s])
    + np.dot(vertices[1].reshape(-1, 1), [1 + r])
    + np.dot(vertices[2].reshape(-1, 1), [1 + s])
)
plt.plot(triangle[:, 0], triangle[:, 1])
plt.scatter(x, y)
...
cubx, cuby = 0.5 * (
    -np.dot(vertices[0].reshape(-1, 1), [cub.r + cub.s])
    + np.dot(vertices[1].reshape(-1, 1), [1 + cub.r])
    + np.dot(vertices[2].reshape(-1, 1), [1 + cub.s])
)
