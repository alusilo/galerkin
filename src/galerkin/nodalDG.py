...
from scipy import linalg as la, special as sf, spatial as sp
...
class NodalDG2D(MeshReader):
            ...
            with open(self.param_file, "rt") as f:
                for line in f.readlines():
                    if not line.startswith("#"):
                        data.append(line)
                self.rho, self.vs, self.vp = np.array(
                    list(map(lambda a: a.split()[:], data[:])), dtype=float
                ).T
            logger.info("Physical parameters file %s loadded successfully.", self.param_file)
        else:
            logger.error("Especified file '%s' does not exists.", self.param_file)
            exit(-1)
    ...
    def GetPMLElements(self):
        logger.info("Identifying elements belonging ABC zone...")
    ...
    def Dmatrices2D(self):
        logger.info("Calculating derivative matrices...")
    ...
    def Nodes2D(self):
        logger.info("Calculating elements nodes...")
        alpopt = [
            0.0000,
            0.0000,
            1.4152,
    ...
    def Lift2D(self):
        logger.info("Calculating fluxes matrices...")
    ...
    def tiConnect2D(self):
        ...
        spNodeToNode = np.concatenate(
            ([index], [np.arange(0, self.Nfaces * K)], [e2e.flatten()], [e2f.flatten()]), axis=0
        )
    ...
    def BuildMaps2D(self):
        logger.info("Mapping elements nodes...")
    ...
    def BuildPNonCon2D(self):
        ...
        logger.info("Initializing DG elements per approximation order...")
        for n in range(Nmax):
            ...
            if self.K > 0:
                ...
                va, vb, vc = self.e2v
                self.x = 0.5 * (
                    -np.dot(VX[va].reshape(-1, 1), [self.r + self.s])
                    + np.dot(VX[vb].reshape(-1, 1), [1 + self.r])
                    + np.dot(VX[vc].reshape(-1, 1), [1 + self.s])
                )
                self.y = 0.5 * (
                    -np.dot(VY[va].reshape(-1, 1), [self.r + self.s])
                    + np.dot(VY[vb].reshape(-1, 1), [1 + self.r])
                    + np.dot(VY[vc].reshape(-1, 1), [1 + self.s])
                )
        ...
        for n1 in range(Nmax):
            ...
            for n2 in range(Nmax):
                ...
                pinfo[n1].interpP[n2] = la.lstsq(
                    mmM, np.dot(interpM.T, np.dot(np.diag(gw), interpP))
                )[0]
