...
for i in range(nf):
	fig, ax = plt.subplots()
	fig.subplots_adjust(left=.16, bottom=.17, right=.98, top=.92)
	ax.plot(time2,data2[i], c='black', linewidth=0.5, label='DGCrack')
	ax.plot(time,traces[i],'r--',c='black',linewidth=0.5,marker='s',markevery=100,markerfacecolor='none', markersize=3, label='nodalDG')
