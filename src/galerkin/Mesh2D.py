...
class MeshReader(object):
	...
	def __plot(self):
		...
		for k in self.surrounding:
			...
			ax.text(cx/1000, cy/1000, ('{}').format(k), fontsize=9, bbox={'facecolor':'red', 'alpha':0.5, 'pad':10})
	...
	def getElementsNearSource(self):
		sx,sy = self.src_position
		print("Info: Indentifying elements on spatial support...")
	...
	def FindSource(self):
		mapS = []
		k=0
		print('Info: Searching for element containing source...')
		while k < self.K:
			...
			if((l1 >= 0.0 and l1 <= 1.0) and (l2 >= 0.0 and l2 <= 1.0) and (l3 >= 0.0 and l3 <= 1.0)):
				mapS.append(k)
				print('Info: Element centroid located at {}[m]'.format(self.centroid(k)))
				break
			k=k+1
		...
		if len(mapS) == 0:
			...
			print('Info: Source found in triangle(s) {}.'.format(mapS))
	...
	def FindGather(self):
		mapG = [[],[]]
		k=0
		print('Info: Searching for element(s) containing gather(s)...')
		while k < self.K:
	...
	def __tri(self):
		if os.path.exists(self.ele_file):
			...
			print("Info: Mesh file {} loadded successfully.".format(self.ele_file))
			print("Info: There were found {} elements.".format(K))
		else:
			print("Error: Especified file '{}' does not exists.".format(self.mesh_file))
			exit(-1)
		...
		if os.path.exists(self.node_file):
			...
			print("Info: Mesh file {} loadded successfully.".format(self.node_file))
			print("Info: There were found {} vertices.".format(Nv))
		else:
			print("Error: Especified file '{}' does not exists.".format(self.mesh_file))
			exit(-1)
