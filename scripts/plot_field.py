#!/usr/bin/env python3
"""Plot wave field animation (run from project root: uv run python scripts/plot_field.py)."""
import json
import os
import sys
...
from galerkin.Mesh2D import MeshReader  # noqa: F403
...
pml_line1 = np.array([[xmin + pml_layer[0], xmin + pml_layer[0]], [ymin, ymax]])
pml_line2 = np.array([[xmin, xmax], [ymax - pml_layer[3], ymax - pml_layer[3]]])
pml_line3 = np.array([[xmax - pml_layer[1], xmax - pml_layer[1]], [ymax, ymin]])
...
obj = MeshReader(mesh_file=mesh_file, src_position=source, gather=gather, src_smooth=0, plot=False)
...
im = plt.imshow(
    frame,
    extent=[xmin / 1000, xmax / 1000, ymax / 1000, ymin / 1000],
    aspect="equal",
